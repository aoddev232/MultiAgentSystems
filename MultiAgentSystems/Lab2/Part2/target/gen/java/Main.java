...
public class Main extends ASTRAClass {
	public Main() {
		...
		addRule(new Rule(
			...
			new Block(
				"Main", new int[] {17,67,17,69},
				new Statement[] {
				}
			)
		));
		addRule(new Rule(
			"Main", new int[] {19,9,19,76},
			new GoalEvent('+',
				new Goal(
					new Predicate("light", new Term[] {
						new Variable(Type.STRING, "lname",false),
		...
		addRule(new Rule(
			...
			Predicate.TRUE,
			new Block(
				"Main", new int[] {24,43,26,5},
				new Statement[] {
					new ModuleCall("c",
						"Main", new int[] {25,8,25,63},
